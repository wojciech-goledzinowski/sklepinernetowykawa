...
if (isset($_GET['akcja']) && $_GET['akcja']=="wez")
{
    if (isset($_GET['potwierdzenie']) && $_GET['potwierdzenie']=="tak")
    {
        ...
        $idzapytania = mysqli_query($db_c, $zapytanie) or die ("blee");
...
<?php
    ...
    $wynik = mysqli_query($db_c, "SELECT `produkty`.`nazwa`, `produkty`.cena, `sprzedaz`.ilosc FROM sprzedaz INNER JOIN produkty ON produkty.id_produktu = sprzedaz.id_produktu WHERE id_zamowienia={$_GET['id_zamowienia']}");
    $razem = 0;
