...
<?php
    ...
    $wynik = mysqli_query($db_c, "SELECT * FROM sprzedaz INNER JOIN uzytkownicy ON sprzedaz.id_uzytkownika = uzytkownicy.id GROUP BY id_zamowienia HAVING potwierdzenie='tak'");
...
<?php
    // odpowiednie zapytanie do bazy o zamówienia danego użytkownika
    $wynik = mysqli_query($db_c, "SELECT * FROM sprzedaz INNER JOIN uzytkownicy ON sprzedaz.id_uzytkownika = uzytkownicy.id GROUP BY id_zamowienia HAVING potwierdzenie='blocked'");
    while($r = mysqli_fetch_assoc($wynik))
    {   // wyświetlenie tabelki z wszystkimi zamówieniami
        $cena = cena_zamowienia($r['id_zamowienia'], $r['id']); // podliczenie ceny danego zamówienia
        echo '<tr>';
