 ...
 */
    ...
    $wynik_kurs_nazwa = mysqli_query($db_c, "SELECT * FROM `produkty` WHERE id_produktu={$id_produktu}");
    while ($r = mysqli_fetch_assoc($wynik_kurs_nazwa)) 
    {
        $b_nazwa = $r['nazwa'];
        $b_cena = $r['cena'];
        $b_ilosc = $r['ilosc'];
    ...
    {   // sprawdzamy, czy pola są wypełnione
        ...
        {         
            ...
            if(mysqli_query($db_c, "UPDATE produkty SET nazwa='{$_POST['nazwa']}', cena='{$_POST['cena']}', ilosc='{$_POST['ilosc']}', opis='{$_POST['opis']}' WHERE id_produktu={$id_produktu}")===TRUE)
            {   
