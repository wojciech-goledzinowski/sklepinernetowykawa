...
// zmiana ilości produktów w zamowieniu
    ...
    {   // musimy zmienić każdą linijkę, więc musimy przejśc przez każdy produkt w petli
        ...
        {  
            ...
            $idzapytania = mysqli_query($db_c, $zapytanie) or die ("blee e e e e e ");
...
global $db_c;
    ...
    $wynik = mysqli_query($db_c, "SELECT `produkty`.`nazwa`, `produkty`.cena, `sprzedaz`.ilosc, `sprzedaz`.id_sprzedazy, produkty.id_produktu FROM sprzedaz INNER JOIN produkty ON produkty.id_produktu = sprzedaz.id_produktu WHERE id_zamowienia={$_GET['id_zamowienia']}");
    $razem = 0;
