...
if($_GET['akcja']=="zablokuj")
{
    ...
    $idzapytania = mysqli_query($db_c, $zapytanie) or die ("Nie da rady anulowac zamowienia. Blad bazy");
...
{
    // TRASAKCJA!!! - start
    mysqli_query($db_c, "START TRANSACTION");
 
    // aktualizacja ilości produktu w bazie
    // przechodzimy przez każdy wpis w ZAMÓWIENIU
    $wynik = mysqli_query($db_c, "SELECT * FROM sprzedaz WHERE id_zamowienia={$_GET['id_zamowienia']}");
    ...
    {   // następnie aktualizujemy każdy wpis  w tabeli PRODUKTY (w magazynie)
        ...
        $idzapytania = mysqli_query($db_c, $zapytanie) or die ("Nie da rady zmienic ilosci. Blad bazy");
    }
    ...
    $idzapytania = mysqli_query($db_c, $zapytanie) or die ("Nie da rady przeniesc zamowienia. Blad bazy");
    
    // TRASAKCJA!!! - stop
    mysqli_query($db_c, "COMMIT");
