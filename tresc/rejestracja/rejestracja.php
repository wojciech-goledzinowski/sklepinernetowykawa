<div class="row">
        <div id="l_panel_lewy" class="col-md-12">
            <legend>Rejestracja</legend>     
        <form action="?v=tresc/rejestracja/rejestracja" id="rejestracja" class="thumbnail" method="post" accept-charset="utf-8">
...
if($licznik_bledow == 0) 
{
    ...
    if($rezultat=mysqli_query($db_c, "SELECT * FROM uzytkownicy WHERE login='$login'")) 
    {
        ...
        if(mysqli_num_rows($rezultat) == 0)
        {
            ...
            if(mysqli_query($db_c, "INSERT INTO uzytkownicy (login, haslo, imie, nazwisko, typ, email) VALUES ('$login','$haslo','$imie','$nazwisko','u','$email')")===TRUE)
            {
...
// jeżeli rejestracja się powiodła należy zalogować użytkownika i wyświetlić panel z informacjami
    if (isset($rejestracja_pomyslna) && $rejestracja_pomyslna==TRUE) 
    {
        ...
        $wynik = mysqli_query($db_c, "SELECT * FROM uzytkownicy WHERE login = '$login' AND haslo = '$haslo'");
