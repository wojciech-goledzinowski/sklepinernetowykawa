...
function cena_zamowienia($id_zamowienia, $id_usera)
{
    global $db_c;
    $cena = 0;
    $wynik = mysqli_query($db_c, "SELECT produkty.cena, sprzedaz.ilosc FROM sprzedaz INNER JOIN produkty ON produkty.id_produktu = sprzedaz.id_produktu WHERE id_zamowienia=$id_zamowienia AND id_uzytkownika=$id_usera");
    while($r = mysqli_fetch_assoc($wynik))
    {
        $cena += $r['cena']*$r['ilosc'];
    }
