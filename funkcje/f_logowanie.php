...
function zaloguj($login, $haslo)
{
    ...
    //$bledy;
     ...
     $czy_zaloguje = mysqli_fetch_array(mysqli_query($db_c, "SELECT COUNT(*) FROM uzytkownicy WHERE login = '$login' AND haslo = '$haslo'")); 
...
{
    global $db_c;
    $wynik = mysqli_query($db_c, "SELECT * FROM uzytkownicy WHERE id={$id_usera}");
    while($r = mysqli_fetch_assoc($wynik))
    {
